...
class AnnotationReadModal(ModalScreen):
    ...
    BINDINGS = [("escape", "dismiss_modal", "Cerrar")]

    def __init__(self, book_name, chapter, verse, text):
        super().__init__()
        self._ref  = f"{book_name} {chapter}:{verse}"
        self._text = text
    ...
    def compose(self) -> ComposeResult:
        with Vertical(id="ar-box"):
            ...
            with VerticalScroll(id="ar-scroll"):
                yield Static(self._text, id="ar-body")
            with Horizontal(id="ar-foot"):
                yield Button("Cerrar [Esc]", id="ar-close", variant="primary")

    def on_mount(self):
        self.query_one("#ar-scroll", VerticalScroll).focus()
...
class AnnotationWriteModal(ModalScreen):
    ...
    #aw-area  { height: 14; border: solid $primary; margin-bottom: 1; }
    ...
    def compose(self) -> ComposeResult:
        with Vertical(id="aw-box"):
            yield Label(
                f"✦  Nota de autor — {self.book_name} {self.chapter}:{self.verse}",
                id="aw-title"
            )
            yield TextArea(self._existing, id="aw-area")
            with Horizontal(id="aw-foot"):
                if self._existing:
                    yield Button("🗑 Eliminar", id="aw-del", variant="error")
                yield Button("💾 Publicar",  id="aw-save",   variant="primary")
                yield Button("Cancelar",     id="aw-cancel")

    def on_mount(self):
        self.query_one("#aw-area", TextArea).focus()

    @on(Button.Pressed, "#aw-save")
    def save(self):
        text = self.query_one("#aw-area", TextArea).text.strip()
        if text:
            ann.save_local(self.translation, self.book_id, self.chapter, self.verse, text)
        self.dismiss(True)

    @on(Button.Pressed, "#aw-del")
    def delete(self):
        ann.delete_local(self.translation, self.book_id, self.chapter, self.verse)
        self.dismiss(True)
